...
class PreflightResult:
    ...
    @property
    def ok(self) -> bool:
        """True when the duplication may proceed (the §9 checks passed)."""
...
def _routing(
    source_ad: Ad, destination_stage: Stage, config: Config
) -> tuple[Optional[StageEntry], Optional[str]]:
    """§9 B: derive the destination through the stage map, never by inference.

    Returns the destination's stage-map entry, or the blocking reason when
    the source campaign is unmapped or the market has no destination for the
    target stage. Name similarity is never consulted (framework.md §4).
    """
    source_entry = (
    ...
    dest = config.destination_for(destination_stage, source_entry.market)
    if dest is None:
        return None, (
            f"market {source_entry.market} has no {destination_stage.value} "
            "destination campaign in the stage map — reported as a proposal, "
            "not duplicated (§9 B)."
        )
    return dest, None


def _destination_adset(
    dest: StageEntry, adsets: list[AdSet], destination_stage: Stage, now: datetime
) -> tuple[Optional[str], Optional[str]]:
    """Resolve the destination ad set: the Reserve "Retired winners" set for
    retirements, the current-month cohort for graduations (created if absent,
    born PAUSED — §5 action).

    Returns ``(existing_adset_id, adset_to_create)`` — exactly one is set.
    """
...
def _blocking_copy(existing: list[Ad], source_ad: Ad) -> Optional[Ad]:
    """§9 A: scan the destination campaign's full enumeration for the first
    non-ARCHIVED copy of this post — even a PAUSED copy blocks (§9 A4:
    re-creating it silently overrides a decision someone already made)."""
...
def preflight(
    ...
    used. Failing either check means **do not create the ad**.
    """
    base: dict[str, Any] = {
    ...
    dest, routing_reason = _routing(source_ad, destination_stage, config)
    if dest is None:
        return PreflightResult(
            status=BLOCKED_ROUTING,
            reasons=[routing_reason or "destination unresolvable (§9 B)"],
            evidence={
                **base,
    ...
    dest_adset_id, adset_to_create = _destination_adset(
        dest, adsets, destination_stage, now
    )

    # --- A: idempotency on post ID across the WHOLE destination campaign -------
    existing = adapter.list_ads_in_campaign(dest.campaign_id)
    blocking = _blocking_copy(existing, source_ad)
    if blocking is not None:
        reason = (
            f"post {source_ad.post_id} already present in destination campaign "
            f"{dest.campaign_id} as ad {blocking.id} (status {blocking.status})."
        )
        if (blocking.status or "").upper() == "PAUSED":
            # §9 A4: a paused copy still skips — surfaced as a proposal, not
            # re-created.
            reason += (
        ...
        return PreflightResult(
            ...
            existing_ad_id=blocking.id,
            existing_ad_status=blocking.status,
            reasons=[reason],
            evidence={
                **base,
